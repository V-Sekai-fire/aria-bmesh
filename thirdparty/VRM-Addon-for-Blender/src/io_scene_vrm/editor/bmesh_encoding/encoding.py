...
class BmeshEncoder:
    ...
    def add_metadata_fallback(self, extension_data: Dict[str, Any], mesh: bpy.types.Mesh) -> Dict[str, Any]:
        """
        Add metadata fallback to extension data for robust reconstruction.
        
        This provides a metadata-based approach when buffer access fails,
        ensuring non-manifold boundary representation can still be reconstructed.
        """
        if not extension_data:
            return extension_data

        logger.info("Adding metadata fallback for non-manifold reconstruction")
        
        metadata = {}

        # Add vertex position metadata
        if mesh.vertices:
            vertex_positions = []
            for vertex in mesh.vertices:
                vertex_positions.extend([vertex.co.x, vertex.co.y, vertex.co.z])
            metadata["vertex_positions"] = vertex_positions
            logger.debug(f"Added {len(mesh.vertices)} vertex positions to metadata")

        # Add edge connectivity metadata
        if mesh.edges:
            edge_vertices = []
            for edge in mesh.edges:
                edge_vertices.extend([edge.vertices[0], edge.vertices[1]])
            metadata["edge_vertices"] = edge_vertices
            logger.debug(f"Added {len(mesh.edges)} edge connections to metadata")

        # Add face connectivity metadata
        if mesh.polygons:
            face_vertices = []
            face_offsets = [0]  # Start with offset 0
            
            for poly in mesh.polygons:
                # Add vertices for this face
                face_vertices.extend(poly.vertices)
                # Add offset for next face
